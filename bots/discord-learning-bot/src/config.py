...
# ============================================================
#  COMMUNITY RULES (canonical source of truth)
# ============================================================
# The pinned #rules content. Lives here (not in scripts/setup_server.py) so BOTH
# the server-setup script AND the bot's /repost-rules command use the exact same
# text — no drift. Bilingual (English + Egyptian Arabic). Attributed to MACAL
# EMPIRE (parent brand); EEC (Empire English Community) is the branch it governs.
...
def chunk_message(content: str, limit: int = DISCORD_MSG_LIMIT) -> list:
    """Split `content` into ≤`limit`-char chunks on paragraph/line boundaries,
    so long pinned content (e.g. the bilingual #rules) posts as several messages
    instead of failing Discord's 2000-char limit. Splits on blank lines first
    (keeps rules whole), then single newlines, and only hard-splits a single
    oversized line as a last resort. Returns [content] when it already fits."""
    content = content or ""
