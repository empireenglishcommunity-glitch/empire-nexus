...
def test_synth_line_for_routes_to_isolated_when_worker_set(monkeypatch):
    mod = _load_rv2()
    seen = {}

    def fake_isolated(text, ch, level, kind):
        seen["kind"] = kind
        return np.ones(200, dtype="float32")

    monkeypatch.setattr(mod, "_worker_python_for",
                        lambda k: "/venvs/x/bin/python")
    monkeypatch.setattr(mod, "_synth_line_isolated", fake_isolated)
    # clone-engine character must route to the "clone" worker
...
def test_synth_line_for_falls_back_when_isolated_fails(monkeypatch):
    ...
    def fake_synth_line(text, ch, level, kokoro, clone):
        calls["in_process"] += 1
        return np.ones(50, dtype="float32")

    monkeypatch.setattr(mod, "synth_line", fake_synth_line)
    ch = {"key": "narrator", "display": "Narrator", "engine": "kokoro"}
    out = mod.synth_line_for("hi", ch, "A2", kokoro=object(), clone=None)
    assert calls["in_process"] == 1 and len(out) == 50


def test_isolated_reads_worker_wav_and_resamples(monkeypatch, tmp_path):
    """_synth_line_isolated shells out, then loads the worker's WAV and resamples to
    SR. We fake the subprocess so no real engine/venv is needed."""
    mod = _load_rv2()
    monkeypatch.setattr(mod, "_worker_python_for", lambda k: "/venvs/x/bin/python")

    # Fake subprocess.run: write a 16kHz wav where the worker's --out is, return ok.
    class _R:
        returncode = 0
        stderr = ""
    ...
    def fake_run(cmd, capture_output=True, text=True, timeout=None):
        ...
        sf.write(out, np.ones(16000, dtype="float32"), 16000)   # 1s @ 16k
        return _R()

    # _synth_line_isolated does `import subprocess` internally, so patching the real
    # module's run() intercepts the call.
    import subprocess as _sp
    monkeypatch.setattr(_sp, "run", fake_run)

    ch = {"key": "narrator", "display": "Narrator", "engine": "kokoro"}
    y = mod._synth_line_isolated("hello", ch, "A2", "kokoro")
    assert y is not None
    # 1s @16k resampled to SR seconds -> ~SR samples
    assert abs(len(y) - mod.SR) <= 2
