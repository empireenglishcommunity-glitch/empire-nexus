...
- [ ] **A1** Rename the 3 modes with purpose + one-line hint: Flashcards /
  ✍️ Translate (Arabic→English) / 🎧 Listen & Type. (`generate.py`)
- [ ] **A2** Forgiving answer matching in `app.js` `checkAnswer`: trim,
  lowercase, strip surrounding punctuation, US/UK spelling map, Levenshtein
  ≤ 1 tolerance with a gentle "almost" note. (R3.2)
- [ ] **A3** Add 🔊 replay control to Translate + Listen modes. (R3.3)
- [ ] **A4** End screen: rename restart → "🔄 Review again"; add "✅ Done".
  (R3.4)
- [ ] **A5** Regenerate pages, `verify_pages.py`, deploy, live-verify a full
...
- [ ] **B1** New `voice_minutes(discord_id, date, minutes)` table + accessors;
  update on voice-state changes so it survives restarts. (R5.3)
- [ ] **B2** `verify_community` requires BOTH (10+ voice min today AND a
  `#general-chat` message today). (R5.1)
- [ ] **B3** `!done community` returns a bilingual checklist of what's done /
  still pending. (R5.2)
- [ ] **B4** Update the community task description / onboarding text. (R5.4)
- [ ] **B5** Tests + live-verify (voice only → pending; chat only → pending;
...
- [ ] **C1** `generate.py`: add `gen_speaking` (record → Send to Discord
  pattern, recording-required note, Home/bottom-nav/day-menu include it).
- [ ] **C2** `api_server.py`: accept `speaking` in the recording-exercise set;
  mastery records the viewed day (already correct post-#239).
- [ ] **C3** `darb.js`: recognise `speaking` in the exercise URL regex; add
  it to the day menu + calendar exercise list.
- [ ] **C4** "Green" rule: keep green = 4 core exercises; Speaking is an
...
- [ ] **C5** Regenerate all pages, `verify_pages.py`, deploy page + bot,
...
- [ ] **D1** Restructure `format_daily_post_chunks`: self-study section
  ...
  (R2.1, R2.3)
- [ ] **D2** Remove any wording implying a specific day-of-week self-study
  lesson that conflicts with personal calendars. (R2.4)
- [ ] **D3** Update daily-message tests for the new format.
