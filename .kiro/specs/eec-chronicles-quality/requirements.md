...
| RC3 | **Audible hard cuts.** Every line is chunked, each chunk edge-trimmed, then concatenated with **no crossfade**. Each join is a click/cut. | Abrupt waveform discontinuities per episode: **386 / 338 / 237 / 303** (Ep1–Ep4) ≈ 12 per spoken line. |
...
| **Naturalness** | mean spectral flatness **≤ 0.045** per voice segment | Empirically separates the accepted voice (0.031) from the rejected one (0.077). |
| **Hard cuts** | **≤ 2** waveform discontinuities per episode | Down from a measured 237–386. Targets RC3. |
