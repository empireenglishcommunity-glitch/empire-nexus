...
- [ ] **P0.1** Add `pronunciation_scores` table to `database.py` schema.
  Add migration for existing databases (ALTER TABLE if not exists).
- [ ] **P0.2** Add `GROQ_WHISPER_MODEL` config to `config.py`
  (default: `whisper-large-v3`).
- [ ] **P0.3** Create `src/pronunciation_scorer.py` with:
...
- [ ] **P0.4** Add `store_pronunciation_score()` and
  `get_recent_scores(discord_id, days=7)` to `database.py`.
- [ ] **P0.5** Unit test the word comparison algorithm with 10+ cases
  (exact match, partial, completely wrong, empty, extra words, etc.)

## Phase P1 — Wire Into !done Flow

- [ ] **P1.1** In `bot.py` `cmd_done`: after `verify_audio()` succeeds
...
- [ ] **A0.1** Add `difficulty_level` column to `members` table
  (INTEGER DEFAULT 2). Add migration.
- [ ] **A0.2** Create `src/adaptive_engine.py` with:
...
- [ ] **A0.3** Call `adaptive_engine.check_and_adjust()` at end of
  `_score_and_notify()` (after storing the new score).
- [ ] **A0.4** If difficulty changes: DM the student with an encouraging
  message explaining the adjustment (Arabic + English).

## Phase A1 — Difficulty Affects Content

- [ ] **A1.1** In `tasks.py` `generate_daily_tasks()`: read member's
...
- [ ] **A1.2** In the practice platform URL generation: optionally pass
  a `?speed=` parameter that the web platform respects for TTS rate.
- [ ] **A1.3** Update `!progress` / `!level` to show current difficulty
  level with an appropriate emoji.
- [ ] **A1.4** Enable `tatawwur_adaptive` flag. Monitor for 1 week.

## Phase A2 — Refinement (after 2+ weeks of data)

- [ ] **A2.1** Analyze real scoring data: are thresholds (85%/50%)
  appropriate? Adjust if too aggressive or too lenient.
- [ ] **A2.2** Add hysteresis: don't oscillate difficulty every day.
  Require 3 consecutive days above/below threshold to change.
- [ ] **A2.3** Add `!difficulty` command so students can see and
  optionally override their difficulty level.
- [ ] **A2.4** Weekly admin report: how many students at each difficulty
